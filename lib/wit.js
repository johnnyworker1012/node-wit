...
function Wit(opts) {
	console.log("Hello! I am entering Wit...");
  ...
  this.message = (message, context) => {
	...
	this.speech = (speech, context) => {
    const method = 'POST';
    const fullURL = witURL + '/speech';
    const handler = makeWitResponseHandler(logger, 'speech');
    logger.debug(method, fullURL);

		let speechHeaders = Object.assign({}, headers);
		delete speechHeaders['Accept']; 
		speechHeaders['Content-Type'] = 'audio/wav';
